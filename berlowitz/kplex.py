...
parser.add_argument('--mode',
                    help='Mode of the algo to run Enum or EnumIncExc. if experiment=results also the nide EnumIncExcNotConnected and EnumNotConnected are available',default="EnumIncExc")
...
def differentNumberOfResultsEnumInExc():
    b = open('diffrent_number_of_results.csv', 'w')
    a = csv.writer(b)
    data = [["Number of results", "EnumIncExc", "Enum"]]
    print "Building a random graph with 1000000 nodes and 10000000 edges."
...
def differentNumberOfResultsEnum():
    b = open('diffrent_number_of_results.csv', 'w')
    a = csv.writer(b)
    data = [["Number of results", "Enum"]]
    print "Building a random graph with 1000000 nodes and 10000000 edges."
    G1 = snap.GenRndGnm(snap.PUNGraph, 1000000, 10*1000000)
    neighbors_dic = InitNeighbors(G1)
    num_of_nodes = 1000000
...
def differentNumberOfResultsEnumNotConnected():
    ...
    a.writerows(data)
    b.close()
    pretty_file.pretty_file("differentNumberOfResultsEnumNotConnected.csv", header=True, border=True, delimiter=",", new_filename ="differentNumberOfResultsEnumNotConnected.txt")
...
def differentNumberOfResultsEnumInNotConnected():
    ...
    a.writerows(data)
    b.close()
    pretty_file.pretty_file("differentNumberOfResultsEnumInNotConnected.csv", header=True, border=True, delimiter=",", new_filename ="differentNumberOfResultsEnumInNotConnected.txt")

def differentNumberOfResultsWithAllTypes():
    b = open('diffrent_number_of_results_for_all_types.csv', 'w')
    a = csv.writer(b)
    data = [["Number of results", "EnumIncExcConnected", "EnumConnected", "EnumIncExcUnConnected", "EnumUnConnected"]]
    print "Building a random graph with 1000000 nodes and 10000000 edges."
    G1 = snap.GenRndGnm(snap.PUNGraph, 1000000, 10*1000000)
    neighbors_dic = InitNeighbors(G1)
    for i in (1,10,100,1000,10000):
    ...
    a.writerows(data)
    b.close()
    pretty_file.pretty_file("diffrent_number_of_results_for_all_types.csv", header=True, border=True, delimiter=",", new_filename ="diffrent_number_of_results_for_all_types.txt")
...
def find_kplex(G1):
    ...
    if(args.type=="connected" or args.type=="all"):
        if args.mode == "EnumIncExc":
            ...
            result = prunconnected.run(G1,args.num_of_kplex, neighbors_dic, args.output+"_connected",args.size)
        if args.mode == "Enum":
            noprun.k= args.k
...
G1 =  snap.GenRndGnm(snap.PUNGraph, 10, 20)
f1 = open('my_file2', 'w')
for EI in G1.Edges():
    f1.write( "%d\t%d\n" % (EI.GetSrcNId(), EI.GetDstNId()))
...
if args.experiment == "edges":
    if args.mode == "Enum" :
        differentNumberOfEdgesEnum()
    if args.mode == "EnumIncExc":
        differentNumberOfEdgesEnumInExc()
#differentNumberOfEdges()
    exit()
if args.experiment == "nodes":
    if args.mode == "Enum" :
        differentNumberOfNodesEnum()
    if args.mode == "EnumIncExc":
        differentNumberOfNodesEnumInExc()
    #differentNumberOfNodes()
    exit()
if args.experiment == "results":
    if args.mode == "Enum" :
        differentNumberOfResultsEnum()
    elif args.mode == "EnumIncExc":
        differentNumberOfResultsEnumInExc()
    if args.mode == "EnumIncExcNotConnected":
        differentNumberOfResultsEnumInNotConnected()
    if args.mode == "EnumNotConnected":
        differentNumberOfResultsEnumNotConnected()
...
if args.experiment == "k":
    if args.mode == "Enum" :
        differentNumberOfKEnum()
    if args.mode == "EnumIncExc":
        differentNumberOfKEnumIncExc()
#differentNumberOfK()
    exit()
